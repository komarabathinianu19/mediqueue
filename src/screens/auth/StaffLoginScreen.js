...
export default function StaffLoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    if (!email.trim()) {
      showAlert("Missing Email", "Please enter your registered email.");
      return;
    }
    if (!password.trim()) {
      showAlert("Missing Password", "Please enter your password.");
    ...
    setLoading(true);
    try {
      // Calls POST /api/hospitals/login
      ...
      // Persist token and hospital info for the session
      await AsyncStorage.setItem("token", data.token);
      await AsyncStorage.setItem("hospitalId", data.hospitalId);
      await AsyncStorage.setItem("hospitalName", data.name);
  ...
  const handleSendResetLink = async () => {
    ...
    setResetLoading(true);
    try {
      // TODO: wire up real password reset API when ready
      ...
      >
        ...
        >
          ...
          >
            ...
            <Text style={styles.modalTitle}>Forgot Password</Text>
            <Text style={styles.modalSub}>
              Enter your registered Hospital ID and Email. We will send password reset instructions.
            </Text>
