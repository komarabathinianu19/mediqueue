...
export default function BookTokenScreen({ route, navigation }) {
  ...
  // Derive departments from actual loaded doctors so the list always matches reality
  const departments = useMemo(() => {
    if (allDoctors.length > 0) {
      const fromDoctors = [...new Set(allDoctors.map((d) => d.department).filter(Boolean))];
  ...
  useEffect(() => {
    ...
    getDoctorsByHospital(hospitalId)
      .then((docs) => {
        setAllDoctors(docs);
        if (docs.length > 0) {
          // If no department pre-selected or current dept has no doctors, pick first doctor's dept
          const firstDoc = selectedDepartment
            ? docs.find((d) => d.department === selectedDepartment) || docs[0]
            : docs[0];
          if (firstDoc) {
            setDepartment(firstDoc.department);
            if (!doctor) setDoctor(firstDoc.name);
          }
        }
      })
      .finally(() => setLoadingDocs(false));
