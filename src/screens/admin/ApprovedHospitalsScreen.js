...
import { Ionicons } from "@expo/vector-icons";
import { COLORS } from "../../constants/colors";
// ✅ FIXED: use fetchApprovedHospitals (not fetchAllHospitals)
import { fetchApprovedHospitals } from "../../services/apiService";
...
export default function ApprovedHospitalsScreen({ navigation }) {
  ...
  const loadApprovedHospitals = async () => {
    try {
      // ✅ Directly hits /api/hospitals/approved — only returns APPROVED ones
      const hospitals = await fetchApprovedHospitals();
      setApprovedHospitals(hospitals);
    } catch (err) {
      console.log("ApprovedHospitals fetch failed:", err.message);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Approved Hospitals</Text>
      <Text style={styles.sub}>
        {approvedHospitals.length} verified hospital
        {approvedHospitals.length !== 1 ? "s" : ""} visible to patients
      </Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyCard}>
            ...
            />
            <Text style={styles.emptyTitle}>No approved hospitals yet</Text>
            <Text style={styles.emptySubtitle}>
              Approve hospitals from the Pending tab
            </Text>
          </View>
        }
        renderItem={({ item }) => (
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              {/* Show department count if available */}
              {item.departments && (
                <Text style={styles.deptText}>
                  <Ionicons name="layers-outline" size={12} />{" "}
                  {item.departments}
                </Text>
              )}
